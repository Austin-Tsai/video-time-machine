document.addEventListener("DOMContentLoaded", () => {
  ...
  chrome.storage.local.get(["defaultSpeed", "toggleSpeed", "toggleKey", "skipAmount", "skipBack", "skipForward"], (saved) => {
    if (saved.defaultSpeed) {
      defaultSpeed.value = saved.defaultSpeed;
    }
    if (saved.toggleSpeed) {
      toggledSpeed.value = saved.toggleSpeed;
    }
    if (saved.toggleKey) {
      toggleKey.value = saved.toggleKey;
    }
    if (saved.skipAmount) {
      skipAmount.value = saved.skipAmount;
    }
    if (saved.skipBack) {
      skipBack.value = saved.skipBack;
    }
    if (saved.skipForward) {
      skipForward.value = saved.skipForward;
    }
  });
